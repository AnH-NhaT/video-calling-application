...
// .trim is eliminate character space left			
				{
					String data = "send chat|"+YourName+"|"+targetName+"*%%"+txtMessage.getText();
					dataSocket.sendData(data);
					txtMessage.setText("");
// configurate string data when push to server					
				}
			}
		});
// create action including statements so that check and send message
...
// + show list users including avatar and name
...
//figure out position or size of image
...
// + create button so that choose a people who you want to chat
...
// if online will replace icon offline default by icon online
				}
			}
		}
	}
// load messages of user1 with user2
...
// classify message of send user or user received
					tagetChat = new JPanel();
//+ if received, the message will be default the left
...
// if user send, will be the right
